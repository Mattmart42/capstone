  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-center w-full">
        <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-400 border-dashed rounded-lg cursor-pointer bg-200 hover:bg-300">
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <svg className="w-8 h-8 mb-4 text-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"/>
            </svg>
            <p className="mb-2 text-sm text-500"><span className="font-semibold">Click to upload</span> or drag and drop</p>
            <p className="text-xs text-500">PDF only (MAX. 5MB)</p>
          </div>
      ...
      <button
        ...
        className="w-full rounded-md bg-primary px-4 py-2 text-white hover:bg-primary-hover disabled:opacity-50"
      >
        {uploading ? 'Uploading...' : 'Analyze Resume'}
      </button>

      {message && <p className="text-center text-sm font-medium text-text">{message}</p>}
    </div>
  )
}